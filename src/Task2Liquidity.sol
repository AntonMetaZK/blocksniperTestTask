...
contract Task2Liquidity is ReentrancyGuard {
    ...
    function buyAndPool(uint256 amountTokenDesired, address[] calldata path, uint256 deadline)
        external
        payable
        nonReentrant
        returns (uint256 liquidity)
    {
        ...
        uint256[] memory amounts =
            router.swapETHForExactTokens{value: msg.value}(amountTokenDesired, path, address(this), deadline);
        ...
        (uint112 reserveToken, uint112 reserveBNB) = (path[1] < WBNB) ? (reserve0, reserve1) : (reserve1, reserve0);

        uint256 optimalTokenAmount = (bnbLeft * reserveToken) / reserveBNB;
        uint256 usedTokenAmount = optimalTokenAmount > amountTokenDesired ? amountTokenDesired : optimalTokenAmount;
        ...
        (,, liquidity) = router.addLiquidityETH{value: usedBNBAmount}(
            path[1], usedTokenAmount, minTokenAmount, minBNBAmount, msg.sender, deadline
        );
